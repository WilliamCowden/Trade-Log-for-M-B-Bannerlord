...
export class ProductManager {
  ...
  async buy(productName, productQuantity, price) {
    await newQuery(
      `UPDATE inventory SET stock = stock + ${productQuantity}, totalinvested = totalinvested + '${price}' WHERE name = '${productName}';`
    );
  };
  ...
  async sell(productName, productQuantity, price = 0) {
    if (productQuantity == false) {
      await newQuery(
        `UPDATE inventory SET stock = 0, totalinvested = totalinvested - '${price}' WHERE name = '${productName}';`
      );
    } else {
      await newQuery(
        `UPDATE inventory SET stock = stock - ${productQuantity}, totalinvested = totalinvested - '${price}' WHERE name = '${productName}';`
